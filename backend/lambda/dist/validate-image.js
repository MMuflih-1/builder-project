...
const handler = async (event) => {
    try {
        ...
        if (hasNonDogAnimal) {
            const detectedAnimal = labels.find(label => nonDogAnimals.some(animal => label.Name?.toLowerCase().includes(animal.toLowerCase())) && (label.Confidence || 0) > 60);
            return {
                statusCode: 400,
                headers: {
                    'Access-Control-Allow-Origin': '*',
        ...
        if (!hasDogIndicators) {
            return {
                ...
                body: JSON.stringify({
                    error: 'Please upload an image of a labrador retriever dog. This image does not appear to contain a dog.',
                    isValid: false
                }),
            };
        }
