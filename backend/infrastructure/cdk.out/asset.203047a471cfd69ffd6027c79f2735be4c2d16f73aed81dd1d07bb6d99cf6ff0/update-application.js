...
const send_sms_1 = require("./send-sms");
...
const handler = async (event) => {
    try {
        ...
        try {
            ...
            const smsData = {
                phoneNumber: getResult.Item.adopterPhone,
                dogName: dogName,
                shelter: shelter,
                status: status,
                applicantName: getResult.Item.adopterName
            };
            console.log('Calling sendApplicationStatusSMS with:', smsData);
            const smsResult = await (0, send_sms_1.sendApplicationStatusSMS)(smsData);
            console.log('SMS result:', smsResult);
            console.log(`SMS notification sent to ${getResult.Item.adopterPhone}`);
        }
        catch (emailError) {
